...
def plot_flare(x, y, x_scatter, y_scatter, yerr, xlim):
    ...
    ax.scatter(x_scatter, y_scatter, color='#000000', s=np.pi * (3) ** 2, alpha=1)  # , label=label_y[v], zorder=zrdr):
    ax.errorbar(x_scatter, y_scatter, yerr=yerr, fmt='None', ecolor='#000000', elinewidth=2, capsize=2, capthick=2)  # , alpha=1, zorder=zrdr)
...
def save_results(lc_parameters, truth, fit, fit_upper, fit_lower, obs, obs_upper, obs_lower,
                 ...
                 flare_start, flare_end, fit_tpf, true_tpf, fit_profile, Test=False):
    ...
    if Test == False:
        if fit_profile == "Convolution":
            file_name = '/Users/lbiddle/PycharmProjects/FlaresResearchNote/Fit_Data_Convolution.csv'
        if fit_profile == "D14":
            file_name = '/Users/lbiddle/PycharmProjects/FlaresResearchNote/Fit_Data_D14.csv'
    if Test == True:
        if fit_profile == "Convolution":
            file_name = '/Users/lbiddle/PycharmProjects/FlaresResearchNote/Test/Fit_Data_Convolution.csv'
        if fit_profile == "D14":
            file_name = '/Users/lbiddle/PycharmProjects/FlaresResearchNote/Test/Fit_Data_D14.csv'
...
test_SNRs = [3., 4., 6., 8., 10., 16., 32., 64., 128., 256.]
...
do_plots = False
for bep in range(1000):
    for rate in test_sampling_rates:
        for SNR in test_SNRs:
            ...
            rise_cadence_resolution_obs, decay_cadence_resolution_obs, total_cadence_resolution_obs = calc_rise_decay_cadence_resolution(flare_start=flare_start_time, flare_stop=flare_stop_time, tpeak=x_data[y_data == max(y_data)][0], sample_rate=rate)
            ...
            rise_point_resolution_obs, decay_point_resolution_obs, total_point_resolution_obs = calc_rise_decay_point_resolution(x=x_data, flare_start=flare_start_time, flare_stop=flare_stop_time, tpeak=x_data[y_data == max(y_data)][0])
            ...
            if do_plots == True:
                ...
                if do_test_plot == True:
                    plot_flare(x=x_truth, y=y_truth, x_scatter=x_data, y_scatter=y_data, yerr=yerr_data, xlim=[start_time, end_time])
            ...
            if len(x_data) > 5:
                ...
                if proceed == True:  #  and (y_data_max_values[2] > 3*np.std(y_data)):
                    ...
                    eqdur_obs, eqdur_obs_upper, eqdur_obs_lower = calc_eqdur_obs(x=x_data, y=y_data, yerr=yerr_data)

                    fwhm_issue = False
                    try:
                        fwhm_obs, fwhm_obs_upper, fwhm_obs_lower = get_fwhm2(x_data, y_data, yerr_data)
                    except:
                        fwhm_obs = float('nan')
                    ...
                    obs_pars = [max(y_data), x_data[y_data == max(y_data)][0], fwhm_obs, eqdur_obs]
                    obs_pars_upper = [max(y_data) + yerr_data[y_data == max(y_data)][0], x_data[y_data == max(y_data)][0], fwhm_obs_upper, abs(eqdur_obs_upper - eqdur_obs)]
                    obs_pars_lower = [max(y_data) - yerr_data[y_data == max(y_data)][0], x_data[y_data == max(y_data)][0], fwhm_obs_lower, abs(eqdur_obs_lower - eqdur_obs)]
                    ...
                    for decision in [False, True]:
                        ...
                        save_results(lc_parameters=lc_pars, truth=true_pars, fit=fit_results, fit_upper=fit_results_upper,
                                     ...
                                     flare_start=flare_start_time, flare_end=flare_stop_time, fit_tpf=tpeak_frac_fit,
                    ...
                    obs_pars = [float('nan'), x_data[y_data == max(y_data)][0], float('nan'), float('nan')]
                    obs_pars_upper = [float('nan'), float('nan'), float('nan'), float('nan')]
                    obs_pars_lower = [float('nan'), float('nan'), float('nan'), float('nan')]
                    ...
                    print(' ')
                    print('------------------')
                    print(str(rate) + ':  Too Low Time Resolution Or SNR')
                    print('------------------')
                    print(' ')
                ...
                print(' ')
                print('------------------')
                print(str(rate) + ':  Too Small Data Train')
                print('------------------')
                print(' ')
